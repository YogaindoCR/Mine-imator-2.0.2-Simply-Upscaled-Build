...
function shader_high_ssao_EX_set(mask)
{
	...
	if (!app.project_render_performance_mode)
		texture_set_stage(sampler_map[?"uMaskBuffer"], surface_get_texture(mask))
	
	render_set_uniform("uPerformanceMode", app.project_render_performance_mode)
	...
	render_set_uniform_int("uSampleStep", app.project_render_ssao_samples)
	render_set_uniform("uRadius", app.project_render_ssao_radius * 2.0)
	render_set_uniform("uPower", app.project_render_ssao_power * 5)
	render_set_uniform("uRatio", app.project_render_ssao_ratio)
	render_set_uniform("uRatioBalance", app.project_render_ssao_ratio_balance)
