	...
	with (new_obj(obj_template))
	{
		...
		if (type_is_shape(type))
		{
			...
			if (ds_map_valid(shapemap))
			{
				...
				shape_major_size = value_get_real(shapemap[?"major_size"], shape_major_size)
				shape_minor_size = value_get_real(shapemap[?"minor_size"], shape_minor_size)
				shape_subdivision = value_get_real(shapemap[?"subdivision"], shape_subdivision)
				shape_morph = value_get_real(shapemap[?"morph"], shape_morph)
