...
float noise(vec2 p)
{
    ...
    float res = mix(mix(hash(ip), hash(ip+vec2(1.0,0.0)), u.x), mix(hash(ip+vec2(0.0,1.0)), hash(ip+vec2(1.0,1.0)), u.x), u.y);
    return res*res;
}
...
// Shadow calculation with subsurface scattering
float calculateShadow(vec2 fragCoord, vec2 bufferMin, float fragDepth, float bias, float att, out vec3 subsurf, out vec3 subsurfHighlight, float sss)
{
    float shadow = 0.0;
    float weightSum = 0.0;
    subsurf = vec3(0.0);
    ...
    if (uLightSize > 0.0001) {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            if (uSSSHighQuality && sss > 0.001) {
                ...
                if (uSSSStrength > 0.01) {
                    ...
                    subsurf += (falloff / (pow(dis, vec3(2.0)) + 1.0) * att * weight) * uSSSStrength;
                }
                ...
                if (uSSSHighlightStrength > 0.01) {
                    ...
                    subsurfHighlight += (falloff / (pow(dis, vec3(2.0)) + 1.0) * att * weight) * uSSSHighlightStrength;
                }
            }
        }
        ...
        float sampleDepth = uLightNear + (uLightFar - uLightNear) * 
                          unpackDepth(getFilteredDepth(fragCoord, bufferMin));
...
void main()
{
    ...
    if (uIsSky > 0) {
        ...
        if (dif > 0.0 || sss > 0.0) {
            ...
            shadow = calculateShadow(fragCoord, bufferMin, fragDepth, bias, att, subsurf, subsurfHighlight, sss);
            
            // Fallback SSS for non-high quality or small light size
            if ((!uSSSHighQuality && sss > 0.001) || (uLightSize <= 0.0001 && sss > 0.001)) {
                vec3 rad, dis, falloff;
                ...
                if (uSSSHighlightStrength > 0.01) {
                    ...
                    falloff = pow(max(1.0 - pow(dis / rad, vec3(uSSSHighlightSharpness)), 0.0), 
                             vec3(uSSSHighlightSharpness * 0.5));
                    subsurfHighlight += (falloff / (pow(dis, vec3(2.0)) + 1.0) * att) * uSSSHighlightStrength;
                }
            }
        }
        ...
        if (sss > 0.001) {
            ...
            float absorption = CSPhase(dot(normalize(vPosition - uCameraPosition), 
                                     normalize(uLightPosition - vPosition)), uAbsorption);
            
            subsurf *= absorption;
            subsurfHighlight *= absorption;
            
            // Mix both subsurface layers
            light += (mix(uLightColor.rgb, vec3(1.0), uSSSDesaturation) * uLightStrength * 
                     uSSSColor.rgb * transDif * subsurf) * smoothstep(0.0, 0.05, (sss / 50.0));
            light += (mix(uLightColor.rgb, vec3(1.0), uSSSHighlightDesaturation) * uLightStrength * 
                     uSSSColor.rgb * transDif * subsurfHighlight) * smoothstep(0.0, 0.05, (sss / 50.0));
