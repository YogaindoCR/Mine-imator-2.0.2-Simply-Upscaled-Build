...
vec4 getColor(vec2 coord, float blur)
{
    ...
    // Simulate bloom-like bokeh on bright areas
    vec3 boostedColor = baseColor.rgb + baseColor.rgb * highlight * blur;

    // Optional: slightly desaturate boosted highlights for realism
    //float desat = dot(boostedColor, vec3(0.333));
    //boostedColor = mix(vec3(desat), boostedColor, 0.85); // 0.85 keeps some color
